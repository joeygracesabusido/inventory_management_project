document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchItemData = async () => {
        ...
        try {
            const responseData = await graphqlFetch(GET_ITEM_QUERY, { itemId });
            console.log('Response Data:', responseData);
            if (responseData.data && responseData.data.getItem) {
                populateForm(responseData.data.getItem);
            } else {
                console.error('Error fetching item data:', responseData.errors);
            }
        } catch (error) {
            console.error('Network error:', error);
        }
    ...
    const populateForm = (item) => {
        ...
        document.getElementById('measurement').value = item.measurement;
        document.getElementById('contacts').value = item.supplier;
        document.getElementById('track_inventory').checked = item.trackInventory;
        document.getElementById('purchase').checked = item.purchase;
        document.getElementById('cost_price').value = item.costPrice;
        document.getElementById('purchase_account').value = item.purchaseAccount;
        document.getElementById('purchase_tax_rate').value = item.purchaseTaxRate;
        document.getElementById('purchase_description').value = item.purchaseDescription;
        document.getElementById('sell').checked = item.sell;
        document.getElementById('sale_price').value = item.salePrice;
        document.getElementById('sales_account').value = item.salesAccount;
        document.getElementById('sales_tax_rate').value = item.salesTaxRate;
        document.getElementById('sales_description').value = item.salesDescription;
    };
    ...
    editItemForm.addEventListener('submit', async (e) => {
        ...
        const UPDATE_ITEM_MUTATION = `
            mutation UpdateItem($itemId: String!, $itemData: ItemCreate!) {
                updateItem(itemId: $itemId, itemData: $itemData) {
                    id
                }
            }
        `;
        ...
        try {
            const responseData = await graphqlFetch(UPDATE_ITEM_MUTATION, { itemId, itemData });

            if (responseData.errors) {
                alert(`Error updating item: ${responseData.errors[0].message}`);
            } else if (responseData.data && responseData.data.updateItem) {
